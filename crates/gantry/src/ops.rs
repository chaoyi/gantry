...
impl OpLock {
    ...
    pub async fn current_op(&self) -> Option<String> {
        self.inner.lock().await.clone()
    }

    pub async fn try_acquire(&self, op_name: &str) -> Result<OpGuard<'_>> {
...
impl Drop for OpGuard<'_> {
    fn drop(&mut self) {
        // We need to clear the lock. Since Drop is sync, we use try_lock.
        if let Ok(mut lock) = self.lock_ref.try_lock() {
            *lock = None;
        }
    }
}
...
pub async fn apply_probe_result(
    ...
    for att in &outcome.attempts {
        state.events.emit(Event::probe_result(
            probe_ref,
            att.ok,
            Some(att.elapsed_ms),
            att.attempt,
...
pub async fn probe_and_resolve(
    ...
    let probe_futures: Vec<_> = stale_probes
        ...
        .map(|(probe_ref, svc_name, container, probe_config)| {
            ...
            let tx = tx.clone();
            async move {
                let _ = tx.send(Event::probe_result(
                    &cr,
                    false,
                    None,
                    0,
