...
/// Skips .cue files and files that are listed as template sources (they are
/// rendered separately by `process_files`).
pub fn copy_build_context(
    build: &BuildDef,
    base_dir: &Path,
    output_svc_dir: &Path,
    skip_srcs: &[&str],
    ...
    copy_dir_filtered(&context_dir, output_svc_dir, base_dir, skip_srcs)
}
...
fn copy_dir_filtered(
    ...
    for entry in std::fs::read_dir(src_dir)? {
        ...
        if src_path.is_dir() {
            copy_dir_filtered(&src_path, &dst_path, base_dir, skip_srcs)?;
        } else {
            // Skip .cue files and template sources
            let ext = src_path.extension().and_then(|e| e.to_str());
            if ext == Some("cue") {
                continue;
            }
            if let Ok(rel) = src_path.strip_prefix(base_dir)
                && skip_srcs.iter().any(|s| rel == Path::new(s))
            {
                continue;
            }
...
pub fn process_files(files: &[FileDef], base_dir: &Path, output_svc_dir: &Path) -> Result<()> {
    for file in files {
        ...
        // Render dst filename through Tera if it contains template syntax
        let dst_name = if file.dst.contains("{{") {
            render_string(&file.dst, file.context.as_ref())?
        } else {
            file.dst.clone()
        };
        let dst_path = output_svc_dir.join(&dst_name);
...
fn render_string(template: &str, context: Option<&serde_json::Value>) -> Result<String> {
    let mut tera = tera::Tera::default();
    tera.add_raw_template("inline", template)
        .map_err(|e| GantryError::Config(format!("template parse error: {e}")))?;
    ...
    tera.render("inline", &ctx)
        .map_err(|e| GantryError::Config(format!("template render error: {e}")))
}
