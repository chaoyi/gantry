...
def reset():
    ...
    tgt = targets[-1] if targets else 'app'
    for _ in range(3):
...
class Model:
    ...
    def _target_services(self, tgt_name):
        """All services transitively needed by a target."""
        svcs = set()
        visited = set()
        queue = [tgt_name]
        while queue:
    ...
    def check_graph(self, probes, tgts, svcs):
        ...
        # 1d. No probe should be green/stale if service is stopped
        for pk, pi in probes.items():
            svc = pk.split('.')[0]
            if svcs.get(svc) == 'stopped' and pi['state'] in ('green', 'stale'):
                v.append(f"graph.stopped_svc_active_probe: {pk}={pi['state']} but {svc} stopped")

        # 1e. Stale with red dep → should be red
        for pk, pi in probes.items():
    ...
    def check_stability(self, op, before, after, svcs):
        ...
        if op_type in ('stop', 'start', 'restart', 'reprobe_svc') and op_arg:
            upstream = set()
    ...
    def check_contract(self, op, result, probes, tgts, svcs):
        ...
        if op_type == 'converge' and result.get('result') == 'ok':
            ...
            # All target probes should be green
            for pk in self.tgt_probes.get(op_arg, []):
                ps = probes.get(pk, {}).get('state')
                if ps != 'green':
                    v.append(f"contract.converge_ok_probe_not_green: {pk}={ps}")
            # Restarted services should now be green
            for svc_name in result.get('actions', {}).get('restarted', []):
                if svcs.get(svc_name) != 'green':
                    v.append(f"contract.restarted_not_green: {svc_name}={svcs.get(svc_name)}")
            # All services in target scope should be running
            needed = self._target_services(op_arg)
            for svc_name in needed:
                if svcs.get(svc_name) == 'stopped':
        ...
        if op_type == 'converge':
            for svc_name in result.get('actions', {}).get('restarted', []):
                svc_info = self.services.get(svc_name, {})
                if not svc_info.get('restart_on_fail', True):
                    v.append(f"contract.restart_on_fail_violated: {svc_name} restarted but restart_on_fail=false")

        # ── Start/Restart contracts ──
        ...
        if op_type == 'stop' and result.get('result') == 'ok':
            ...
            for pk in probes:
                if pk.startswith(op_arg + '.'):
                    ...
                    # All cross-service reverse deps should not be green
                    for r in self.rdeps.get(pk, []):
        ...
        if op_type == 'reprobe_all' and result.get('result') == 'ok':
            for pk, pi in probes.items():
                svc = pk.split('.')[0]
                if svcs.get(svc) == 'stopped': continue
                if pi['state'] != 'stale': continue
                if pk in self.meta: continue
                all_deps_green = all(
                    probes.get(d, {}).get('state') == 'green'
                    for d in self.deps.get(pk, []))
                if all_deps_green:
                    v.append(f"contract.reprobe_all_unresolved: {pk} stale after reprobe_all with all deps green")
...
def run_op(op):
    ...
    paths = {'stop': f'/stop/service/{a}', 'start': f'/start/service/{a}?timeout=30',
             ...
             'converge': f'/converge/target/{a}?timeout=90',
...
def random_op(services, targets):
    # Weight converge higher — it tests the most interesting paths
    t = random.choices(
        ['stop', 'start', 'restart', 'converge', 'reprobe_all', 'reprobe_svc', 'reprobe_tgt'],
        weights=[2, 2, 1, 3, 1, 1, 1],
    )[0]
